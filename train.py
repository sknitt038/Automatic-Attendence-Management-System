        ...
        for(x,y,w,h) in faces:
            ...
            if(conf < 50):
               ...
               # date = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d')
                ...
                #print (k)
                if t == True or prev!=k:
                    with open('Attendance/empDetails.csv','a+') as csvFile:
                        writer = csv.writer(csvFile)
                        writer.writerow(row)
                    csvFile.close()
                    t = False
...
copyWrite.insert("insert", "Developed by Alucor limited","", "TEAM", "AI")
